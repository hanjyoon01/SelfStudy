...
public class VocManager {
	...
	void makeVoc(String filename) {
		try(Scanner scan = new Scanner(new File(filename))){
			while(scan.hasNextLine()) {
				...
				this.addword(new Word(temp[0].trim(), temp[1].trim(), 0));
			}
	...
	void menu() {
		...
		while(choice!=5) {
			...
			System.out.println("\n--------한재윤의 단어장--------");
			System.out.println("1)단어검색    2)단어검색2  3)객관식 퀴즈    4)빈출단어    5)종료");
			...
			try {
				...
				case 5:
					...
					break;
				default:
					System.out.println("1과 5 사이의 숫자를 입력해 주세요.");
				}
	...
	public void FrequentVoc() {
		
		Collections.sort(list, new FreqComparator());

		for(int i=0;i<5;i++) {
			System.out.println("검색빈도 ("+list.get(i).Freq+") "+list.get(i));
		}
	}
